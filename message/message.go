...
func SendMessage(msgID int, data interface{}) (result map[string]interface{}) {
	...
	tm.PrintDuration(fmt.Sprintf("push to msgCenter %v len=%v", msgID, len(msgCenter.ch)), start, time.Second)
	...
	if ok {
		for name, c := range cs {
			...
			tm.PrintDuration(fmt.Sprintf("call %v msgid=%v", name, msgID), start, time.Second)
		}
	}
	return
}
