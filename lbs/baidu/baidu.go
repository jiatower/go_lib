...
func SearchPlace(lat, lng float64, radius, cur, ps int, sortByDistence bool, keywords ...string) (places []Place, total int, e error) {
	...
	var body []byte
	if sortByDistence {
		body, e = yh_http.Send("http", "api.map.baidu.com", "place/v2/search", map[string]string{"output": "json", "ak": AK, "scope": "1", "page_num": utils.ToString(cur - 1), "page_size": utils.ToString(ps), "location": fmt.Sprintf("%v,%v", lat, lng), "radius": utils.ToString(radius), "query": q, "filter": "sort_name:distance|sort_rule:1"}, nil, nil, nil, 4)
	} else {
		body, e = yh_http.Send("http", "api.map.baidu.com", "place/v2/search", map[string]string{"output": "json", "ak": AK, "scope": "1", "page_num": utils.ToString(cur - 1), "page_size": utils.ToString(ps), "location": fmt.Sprintf("%v,%v", lat, lng), "radius": utils.ToString(radius), "query": q}, nil, nil, nil, 4)
	}
	if e != nil {
		return nil, 0, e
	}
...
func SearchCity(city string, cur, ps int, keywords ...string) (places []Place, total int, e error) {
	...
	q := strings.Join(keywords, "$")
	body, e := yh_http.Send("http", "api.map.baidu.com", "place/v2/search", map[string]string{"output": "json", "ak": AK, "scope": "1", "page_num": utils.ToString(cur - 1), "page_size": utils.ToString(ps), "region": city, "query": q}, nil, nil, nil, 4)
	if e != nil {
		return nil, 0, e
	}
	var result PlaceResult
...
func SearchProvince(province string, cur, ps int, keywords ...string) (cities []CityNum, total int, e error) {
	...
	q := strings.Join(keywords, "$")
	body, e := yh_http.Send("http", "api.map.baidu.com", "place/v2/search", map[string]string{"output": "json", "ak": AK, "scope": "1", "page_num": utils.ToString(cur - 1), "page_size": utils.ToString(ps), "region": province, "query": q}, nil, nil, nil, 4)
	if e != nil {
		return nil, 0, e
	}
...
func SuggestionPlace(region string, keyword string, coordinate ...float64) (sugestions []Suggestion, e error) {
	body := []byte{}
	if len(coordinate) >= 2 {
		body, e = yh_http.Send("http", "api.map.baidu.com", "place/v2/suggestion", map[string]string{"output": "json", "ak": AK, "region": region, "location": fmt.Sprintf("%v,%v", coordinate[0], coordinate[1]), "query": keyword}, nil, nil, nil, 4)
	} else {
		body, e = yh_http.Send("http", "api.map.baidu.com", "place/v2/suggestion", map[string]string{"output": "json", "ak": AK, "region": region, "query": keyword}, nil, nil, nil, 4)
	}
	if e != nil {
		return nil, e
	}
...
func GetCityByIP(ip string) (province, city string, e error) {
	body, e := yh_http.Send("http", "api.map.baidu.com", "location/ip", map[string]string{"ak": AK, "ip": ip}, nil, nil, nil)
	if e != nil {
		return "", "", e
	}
...
func GetGPSByCity(city string, province string) (pos lbs.Coordinate, e error) {
	body, e := yh_http.Send("http", "api.map.baidu.com", "geocoder/v2/", map[string]string{"ak": AK, "output": "json", "address": province + city, "city": city}, nil, nil, nil)
	if e != nil {
		return pos, e
	}
...
func GetCityByGPS(pos lbs.Coordinate) (city, province string, e error) {
	body, e := yh_http.Send("http", "api.map.baidu.com", "geocoder/v2/", map[string]string{"ak": AK, "output": "json", "location": fmt.Sprintf("%v,%v", pos.Lat, pos.Lng)}, nil, nil, nil)
	if e != nil {
		return "", "", e
	}
...
func GetCityByPhone(phone string) (city, province, supplier string, e error) {
	body, e := yh_http.Send("http", "apis.baidu.com", "apistore/mobilephoneservice/mobilephone", map[string]string{"tel": phone}, map[string]string{"apikey": APIKEY}, nil, nil)
	if e != nil {
		return "", "", "", e
	}
