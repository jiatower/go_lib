...
// and predictably.
// 废弃rand2参数，改为在方法体内声明rand 对象，是为了防止并发时导致的错误（index out of range), 需要特别注意
func RandomSpec0(count uint, start, end int, letters, numbers bool, chars []rune, rand2 *rand.Rand) string {
	if count == 0 {
		return ""
	}
	if start == 0 && end == 0 {
