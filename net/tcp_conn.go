...
func (conn *TCPConn) SetTimeout(to uint) {
	...
	if to == 0 {
		...
		conn.conn.SetDeadline(tm.Now.Add(time.Duration(to) * time.Second))
	}
}
...
func (conn *TCPConn) WriteSafe(data []byte) (err error) {
	...
	for length > 0 {
		if conn.writeTimeout > 0 {
			conn.conn.SetWriteDeadline(tm.Now.Add(time.Duration(conn.writeTimeout) * time.Second))
		}
...
func (conn *TCPConn) ReadSafe(data []byte) (err error) {
	...
	for length > 0 {
		if conn.readTimeout > 0 {
			conn.conn.SetReadDeadline(tm.Now.Add(time.Duration(conn.readTimeout) * time.Second))
		}
